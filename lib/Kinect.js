...
(function(window){
  ...
  function FusionKinect(){
    ...
    _fusionKinectObj.setFaceEnabled = function(state){
         settings.faceEnabled = state;         
    };
	...
	_fusionKinectObj.setColorImageEnabled = function(state){
         settings.colorImageEnabled = state;         
    };
    ...
    _fusionKinectObj.getFaceEnabled = function(state){
         return settings.faceEnabled;
    };
	...
	_fusionKinectObj.getColorImageEnabled = function(state){
         return settings.colorImageEnabled;         
    };
    ...
    _fusionKinectObj.getFaceData = function() {
        return faceData;
    }
    ...
    _fusionKinectObj.getThresholdDataAsImage = function() {
	 ...
	 _fusionKinectObj.getColorDataAsImage = function() {
        return colorDataAsImage;
    }

    _fusionKinectObj.requestMultiple = function() {
      if(websocket.readyState != 1) {
        console.debug("Kinect Network connection unavailable - connect or reconnect first");
        return
      } 
      ...
      if(settings.audioEnabled) websocket.send("a");
	    if(settings.colorImageEnabled) websocket.send("c");
      if(settings.faceEnabled) websocket.send("f");
    ...
    _fusionKinectObj.connect = function(ip) {
         ...
         if ("WebSocket" in window) {
               ...
               { 
                  ...
                  if(object.type === 32)        skeletonData = object;
                  else if(object.type === 4)    markerData = object;
                  else if(object.type === 1024) blobData = object;
                  else if(object.type === 768)  thresholdDataAsImage = object;
                  else if(object.type === 8)    depthDataAsImage = object;
				          else if(object.type === 128)  colorDataAsImage = object;
                  else if(object.type === 64)   audioData = object;
                  else if(object.type === 1280) faceData = JSON.parse(object.Data);
                  //console.log(object);
               };
